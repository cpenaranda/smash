...
bool SendInformationToServer(const int &port, const std::string &address,
                             const Options &options,
                             const std::string &library_name,
                             const bool &all_options,
                             const uint8_t &best_options,
                             const uint32_t &best_result_number,
                             const uint32_t &repetitions, NetworkClient *net) {
  bool status = true;
...
int main(int argc, char *argv[]) {
  ...
  if (Utils::GetParamsClient(argc, argv, &opt, &input_file_name,
                             &compression_library_name, &all_options,
                             &best_options, &best_result_number, &repetitions,
                             &port, &address)) {
    ...
    if (SendInformationToServer(port, address, opt, compression_library_name,
          ...
          if (SetMemories(input_file_name, &uncompressed_data,
              ...
              if (result == EXIT_SUCCESS) {
                ...
                if (result == EXIT_SUCCESS) {
                  ...
                  if (result == EXIT_SUCCESS) {
                    ...
                    result =
                        net->SendBuffer(compressed_data, compressed_size, true)
                            ? EXIT_SUCCESS
                            : EXIT_FAILURE;
                    GetDuration(start, &transfer_time);
                    if (result == EXIT_SUCCESS) {
                      result = net->RecvACK() ? EXIT_SUCCESS : EXIT_FAILURE;
                      if (result == EXIT_SUCCESS) {
                        result =
                            net->RecvBuffer(
                                reinterpret_cast<char *>(&decompression_time),
                        ...
                        if (result == EXIT_SUCCESS) {
                          ...
                          total_results.push_back(
                              (static_cast<double>(uncompressed_size) /
                               1000000.0) /
                              (compression_time.count() +
                               transfer_time.count() + decompression_time));
                        } else {
                          std::cout << "ERROR: " << library_name
                                    << " when decompressed data is transfered"
                                    << std::endl;
                        }
                      } else {
                        std::cout << "ERROR: " << library_name
            ...
            if (result == EXIT_SUCCESS) {
              ...
              GetInformation(repetitions, compression_results, transfer_results,
                             ...
                             &error_decompression, &error_total);
      ...
      if (!results.empty()) {
        Utils::ShowTitle(uncompressed_size, repetitions, true);
        if (!best_options) {
