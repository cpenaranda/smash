...
ZlingReader::ZlingReader(const char *buffer, const uint64_t &buffer_size)
    : buffer_(buffer),
...
bool ZlingLibrary::CheckOptions(CpuOptions *options, const bool &compressor) {
  bool result{true};
  if (compressor) {
    result =
        CpuCompressionLibrary::CheckCompressionLevel("zling", options, 0, 4);
  }
  return result;
}

bool ZlingLibrary::Compress(const char *const uncompressed_data,
                            const uint64_t &uncompressed_data_size,
                            char *compressed_data,
                            uint64_t *compressed_data_size) {
  bool result{initialized_compressor_};
  if (result) {
    ZlingReader reader(uncompressed_data, uncompressed_data_size);
    ZlingWriter writer(compressed_data, *compressed_data_size);
    ...
    *compressed_data_size = writer.GetOutputSize();
  }
  return result;
}

bool ZlingLibrary::Decompress(const char *const compressed_data,
                              const uint64_t &compressed_data_size,
                              char *decompressed_data,
                              uint64_t *decompressed_data_size) {
  bool result{initialized_decompressor_};
  if (result) {
    ZlingReader reader(compressed_data, compressed_data_size);
    ZlingWriter writer(decompressed_data, *decompressed_data_size);
